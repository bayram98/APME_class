...
from module10_twitter_streaming.part00_preclass.twitter_streaming import TweetToFileListener
...
def listen_and_store_tweets_lan(keywords, duration, lan, output_file, twitter_credentials):
    ...
    stream.filter(track=keywords, languages = lan ,async=True, )  # track all the tweets in which the keywords 'brexit' and 'Brexit' appear, the async option makes sure that tracking does not block teh execution of the program
    sleep(duration)  # this keeps the tracking alive for 10 seconds
    stream.disconnect()
