...
def hot_potato_spicy(name_list, num):
    ...
    while not stop:
        ...
        if random.random() > 0.5:
            queue.enqueue(s)
            print(" random > 0.5 no any change on {0} {1}".format(s[0] , s[1] ))
        else:
            s[1] = s[1] - 1
            if s[1] <= 0 :
                print(" random <= 0.5 and no any more life {0} dropped".format(s[0]))
            else:
                queue.enqueue(s)
                print(" random <= 0.5 and life decreased {0} {1} dropped".format(s[0],s[1]))
